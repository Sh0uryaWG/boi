...
const transactionPortal = document.getElementById(`transaction-portal`)
console.log(transactionPortal)
function sliderPortal(argument) {
	// body...
	transactionPortal.style.animation = `slider 1.5s ease-in-out forwards `
	pointerList.map(e => {
		e.remove()
	})
}
payment.addEventListener(`click`, sliderPortal)